...
from reyshell.utils.notify import notify
from reyshell.utils.paths import atomic_dump, scheme_data_dir, scheme_path
            ...
            try:
                ...
                if self.notify:
                    notify(
                        ...
                        "No wallpaper set. Please set a wallpaper via `reyshell wallpaper` before setting a dynamic scheme.",
                    )
                raise ValueError(
                    "No wallpaper set. Please set a wallpaper via `reyshell wallpaper` before setting a dynamic scheme."
                )
...
def get_scheme_modes(name: str | None = None, flavour: str | None = None) -> list[str]:
    ...
    if name == "dynamic":
        return ["light", "dark"]
    return [f.stem for f in (scheme_data_dir / name / flavour).iterdir() if f.is_file()]  # type: ignore
