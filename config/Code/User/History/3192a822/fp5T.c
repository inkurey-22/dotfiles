...
void print_upward_cone(int stars)
{
...
void print_body_line(int line_len, int first_star, int second_star)
{
...
void print_body_upper(unsigned int s, int bs)
{
    for (int l = 0; l < (int)s; l++) {
        int first_star = l + 1;
        int second_star = (s == 1) ? 5 : bs - l;
        print_body_line(bs, first_star, second_star);
    }
}

// Lower part of the body
void print_body_lower(unsigned int s, int bs)
{
...
void print_downward_cone(int stars)
{
    for (int k = 1; k <= stars; k++) {
        if (k == 1 || k == stars)
            my_putchar('*');
