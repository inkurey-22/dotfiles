...
fn plus_five(value: &DataValue) -> DataValue {
    match value {
        DataValue::Int(v) => DataValue::Int(v + 5),
        _ => value.clone(), // Return the original value if not an Int
    }
}
...
fn main() {
    ...
    let plusfive = df.apply("age", &plus_five);
    plusfive.write_csv("plus_five.csv").unwrap_or_else(|_| {
        eprintln!("Failed to write plus five data to CSV");
    });

    let wealth_values = df.get_values("wealth");
    println!("Wealth values:");
    match wealth_values {
        bea::DataValue::List(ref values) => {
            for (i, value) in values.iter().enumerate() {
                match value {
                    bea::DataValue::Float(f) => println!("Row {}: {}", i + 1, f),
                    _ => println!("Row {}: {:?}", i + 1, value),
                }
            }
        }
        _ => {
            println!("Wealth column is not a list of values: {:?}", wealth_values);
        }
    }
}
