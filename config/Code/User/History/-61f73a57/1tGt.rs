...
impl DataFrame {
    ...
    pub fn groupby<F>(&self, group_by: &str, to_aggregate: &[&str], agg_func: F) -> DataFrame
    where
        F: Fn(&[DataValue]) -> DataValue,
    {
        let group_col_idx = match self.columns.iter().position(|c| c == group_by) {
            Some(idx) => idx,
            None => return DataFrame::from_rows(self.columns.clone(), self.column_types.clone(), vec![]), // Return empty if column not found
        };

        let mut groups: std::collections::HashMap<DataValue, Vec<Vec<DataValue>>> = std::collections::HashMap::new();

        for row in &self.data {
            let key = row.get(group_col_idx).cloned().unwrap_or(DataValue::Null);
            groups.entry(key).or_default().push(row.clone());
        }

        let mut aggregated_rows = Vec::new();
        for (key, rows) in groups {
            let mut aggregated_row = vec![key];
            for &col in to_aggregate {
                let col_idx = match self.columns.iter().position(|c| c == col) {
                    Some(idx) => idx,
                    None => continue, // Skip if column not found
                };
                let values: Vec<DataValue> = rows.iter().filter_map(|r| r.get(col_idx).cloned()).collect();
                aggregated_row.push(agg_func(&values));
            }
            aggregated_rows.push(aggregated_row);
        }

        let new_columns: Vec<String> = std::iter::once(group_by.to_string())
            .chain(to_aggregate.iter().map(|&s| s.to_string()))
            .collect();

        DataFrame::from_rows(new_columns, self.column_types.clone(), aggregated_rows)
    }
}
