...
{-     let values = [5, 3, 8, 1, 4, 7, 9]
    ...
    print (sum tree)
    putStrLn "\nTesting treeSort with a large randomized list (1,000,000 elements):" -}
    let bigList = [1..1000000]
    randomizedList <- shuffleM bigList
    print (take 10 (treeSort randomizedList))
    putStrLn "... (showing only first 10 elements of sorted list)"