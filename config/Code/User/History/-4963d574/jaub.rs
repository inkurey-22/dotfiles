...
impl DataFrame {
    ...
    pub fn sort(&self, column: &str) -> DataFrame {
        let col_idx = match self.columns.iter().position(|c| c == column) {
            Some(idx) => idx,
            None => return self.clone(), // Return unchanged if column not found
        };
        ...
        sorted_data.sort_by(|a, b| {
            ...
            match (va, vb) {
                ...
                (DataValue::Null, _) => std::cmp::Ordering::Greater, // Nulls last
                (_, DataValue::Null) => std::cmp::Ordering::Less,
