...
myPartition _ [] = ([], [])
myPartition p xs = myFoldr partitioner ([], []) xs
  where
    partitioner x (ts, fs)
      | p x       = (x:ts, fs)
      | otherwise = (ts, x:fs)
...
myQuickSort cmp (x:xs) =
  let first = myFilter (`cmp` x) xs
      second = myFilter (not . (`cmp` x)) xs
  in myQuickSort cmp first ++ [x] ++ myQuickSort cmp second
...
main = do
  ...
  putStrLn "Testing myMap (+1) [1,2,3]:"
  print (myMap (+1) [1,2,3])
  putStrLn "Testing myFilter (>1) [1,2,3]:"
  print (myFilter (>1) [1,2,3])
  putStrLn "Testing myFoldl (+) 0 [1,2,3]:"
  print (myFoldl (+) 0 [1,2,3])
  putStrLn "Testing myFoldr (+) 0 [1,2,3]:"
  print (myFoldr (+) 0 [1,2,3])
  putStrLn "Testing myPartition (>1) [1,2,3]:"
  print (myPartition (>1) [1,-1,2,3,-2])
  putStrLn "Testing myQuickSort (>) [3,1,4,2]:"
  print (myQuickSort (>) [3,1,4,2])
