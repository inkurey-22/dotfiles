...
from reyshell.utils.hypr import message
from reyshell.utils.material.utils import get_colours_for_image
from reyshell.utils.paths import (
    compute_hash,
    wallpaper_link_path,
    wallpaper_path_path,
...
from reyshell.utils.scheme import Scheme, get_scheme
from reyshell.utils.theme import apply_colours
...
def get_wallpapers(args: Namespace) -> list[Path]:
    ...
    for monitor in monitors[1:]:
        filter_size[0] = min(filter_size[0], monitor["width"])
        filter_size[1] = min(filter_size[1], monitor["height"])
...
def get_smart_opts(wall: Path, cache: Path) -> str:
    ...
    from reyshell.utils.colorfulness import get_variant
