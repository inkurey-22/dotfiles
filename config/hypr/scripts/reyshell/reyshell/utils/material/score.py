    ...
    def score(colors_to_population: dict, filter_enabled: bool = False) -> Hct:
        ...
        for hct in colors_hct:
            ...
            if filter_enabled and (
                hct.chroma < Score.CUTOFF_CHROMA or proportion <= Score.CUTOFF_EXCITED_PROPORTION
            ):
                continue

            proportion_score = proportion * 100.0 * Score.WEIGHT_PROPORTION
            chroma_weight = (
                Score.WEIGHT_CHROMA_BELOW if hct.chroma < Score.TARGET_CHROMA else Score.WEIGHT_CHROMA_ABOVE
            )
        ...
        return (
            DislikeAnalyzer.fix_if_disliked(primary) if primary else Score.score(colors_to_population, False)
        )
