...
from reyshell.utils.notify import close_notification, notify
from reyshell.utils.paths import recording_notif_path, recording_path, recordings_dir, user_config_path
...
class Command:
    ...
    def run(self) -> None:
        if self.args.pause:
            subprocess.run(["pkill", "-USR2", "-f", RECORDER], check=False, stdout=subprocess.DEVNULL)
    ...
    def proc_running(self) -> bool:
        return subprocess.run(["pidof", RECORDER], check=False, stdout=subprocess.DEVNULL).returncode == 0
    ...
    def stop(self) -> None:
        ...
        subprocess.run(["pkill", "-f", RECORDER], check=False, stdout=subprocess.DEVNULL)
